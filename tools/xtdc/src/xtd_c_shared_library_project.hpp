...
namespace xtdc_command {
  ...
  public:
    explicit xtd_c_shared_library_project(const xtd::string& path) : base_project(path) {}
  ...
  private:
    ...
    void generate_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      ...
      for (const auto& file : sources)
        lines.add(xtd::string::format("  {}", file));
      lines.add(")");
      lines.add("target_type(C_SHARED_LIBRARY)");
      lines.add("");
      lines.add("# Install");
      lines.add("add_install_include_directories(include)");
