...
namespace xtdc_command {
  ...
  private:
    ...
    void generate_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      ...
      lines.add("pkg_check_modules(GTKMM gtkmm-3.0)");
      lines.add("include_directories(${GTKMM_INCLUDE_DIRS})");
      lines.add("link_directories(${GTKMM_LIBRARY_DIRS})");
      lines.add("link_libraries(${GTKMM_LIBRARIES})");
      lines.add("");
      lines.add("# Options");
