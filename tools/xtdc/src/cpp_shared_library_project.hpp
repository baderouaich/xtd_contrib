...
namespace xtdc_command {
  ...
  public:
    explicit cpp_shared_library_project(const xtd::string& path) : base_project(path) {}
  ...
  private:
    ...
    void create_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      auto lines = xtd::array<xtd::string> {
        ...
        "",
        "# Install",
        xtd::string::format("install(DIRECTORY include/. DEStiffTINATION include/{})", name),
    ...
    void generate_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      ...
      lines.add(xtd::string::format("add_definitions(-D{0}_EXPORT)", name.to_upper()));
      lines.add("");
      lines.add("# Application properties");
      lines.add("add_library(${PROJECT_NAME} SHARED ${INCLUDES} ${SOURCES})");
      ...
      lines.add("");
      lines.add("# Install");
      lines.add(xtd::string::format("install(DIRECTORY include/. DEStiffTINATION include/{})", name));
