...
namespace xtdc_command {
  ...
  public:
    explicit c_static_library_project(const xtd::string& path) : base_project(path) {}
  ...
  private:
    ...
    void generate_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      ...
      lines.add("");
      lines.add("# Install");
      lines.add("if (NOT INSTALL_PROJECT_NAME)");
      lines.add("  set(INSTALL_PROJECT_NAME ${PROJECT_NAME})");
      lines.add("endif()");
      ...
      lines.add("install(FILES $<TARGET_FILE_DIR:${INSTALL_PROJECT_NAME}>/${PROJECT_NAME}${CMAKE_DEBUG_POSTFIX}.pdb DESTINATION lib CONFIGURATIONS Debug OPTIONAL)");
      lines.add("install(FILES $<TARGET_FILE_DIR:${INSTALL_PROJECT_NAME}>/${PROJECT_NAME}.pdb DESTINATION lib CONFIGURATIONS Release OPTIONAL)");
      lines.add("install(TARGETS ${INSTALL_PROJECT_NAME} EXPORT ${PROJECT_NAME} DESTINATION lib)");
      lines.add("install(EXPORT ${INSTALL_PROJECT_NAME} DESTINATION cmake)");
