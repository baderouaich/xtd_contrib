...
namespace xtdc_command {
  ...
  public:
    explicit gtest_unit_test_application_project(const xtd::string& path) : base_project(path) {}
  ...
  private:
    ...
    void generate_cmakelists_txt(const xtd::string& name, const xtd::string& path) const {
      ...
      lines.add("target_link_libraries(${PROJECT_NAME} GTest::GTest GTest::Main)");
      lines.add("");
