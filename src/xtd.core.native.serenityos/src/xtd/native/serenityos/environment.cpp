...
#  warning "The compiler is unknown, please check how to get command line arguments from the compiler."
  ...
  const distribution_dictionary& get_distribution_key_values() {
    ...
    distribution_key_values.insert({"ID", xtd::native::serenityos::strings::replace(xtd::native::serenityos::strings::to_lower(name), " ", "")});
...
string environment::get_desktop_environment() {
  ...
  for (auto environment_desktop : {"budgie", "cinnamon", "deepin", "Enlightenment", "étoilé", "gnome", "kde", "lxqt", "mate", "pantheon", "razor", "unity", "xfce"}) {
    if (serenityos::strings::contains(serenityos::strings::to_lower(current_desktop), environment_desktop)) return environment_desktop;
  }
  return "";
}
...
string environment::get_desktop_theme() {
  ...
  auto current_theme = serenityos::shell_execute::run("gsettings", "get org.gnome.desktop.interface gtk-theme");
  if (current_theme.size() >= 4)
    current_theme = serenityos::strings::substring(current_theme, 1, current_theme.size() - 3);
...
vector<string> environment::get_distribution_like_ids() {
  ...
  if (iterator == get_distribution_key_values().end()) return {};
  return serenityos::strings::split(iterator->second, {' '});
}
...
void environment::get_distribution_version(int32_t& major, int32_t& minor, int32_t& build, int32_t& revision) {
  ...
  if (name_it == get_distribution_key_values().end()) return;
  auto versions = xtd::native::serenityos::strings::split(name_it->second, {'.'});
  ...
  if (target == ENVIRONMENT_VARIABLE_TARGET_PROCESS) {
    ...
    if (envs.size() == 0) {
      for (size_t index = 0; environ[index] != nullptr; index++) {
        vector<string> key_value = serenityos::strings::split(environ[index], {'='});
        if (key_value.size() == 2)
...
string environment::get_machine_name() {
  return serenityos::strings::replace(serenityos::shell_execute::run("uname", "-n"), "\n", "");
}
...
void environment::get_os_version(int32_t& major, int32_t& minor, int32_t& build, int32_t& revision) {
  auto numbers = serenityos::strings::split(serenityos::shell_execute::run("uname", "-r"), {'.', '-', '\n'});
  if (numbers.size() < 1 || !serenityos::strings::try_parse(numbers[0], major)) major = 0;
  if (numbers.size() < 2 || !serenityos::strings::try_parse(numbers[1], minor)) minor = 0;
  if (numbers.size() < 3 || !serenityos::strings::try_parse(numbers[2], build)) build = 0;
  if (numbers.size() < 4 || !serenityos::strings::try_parse(numbers[3], revision)) revision = 0;
}
...
string environment::get_user_domain_name() {
  return serenityos::strings::trim_end(serenityos::shell_execute::run("uname", "-n"), {'\n'});
}
...
bool environment::has_shutdown_started() {
  // return always false on serenityos.
  return false;
}

bool environment::is_processor_arm() {
  auto uname_result = serenityos::shell_execute::run("uname", "-m");
  return serenityos::strings::contains(uname_result, "arm") || serenityos::strings::contains(uname_result, "aarch64");
}

bool environment::is_os_64_bit() {
  return serenityos::strings::contains(serenityos::shell_execute::run("uname", "-m"), "64");
}
