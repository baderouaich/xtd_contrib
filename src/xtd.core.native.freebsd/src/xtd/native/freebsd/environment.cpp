...
#include "../../../../include/xtd/native/freebsd/strings.hpp"
#include "../../../../include/xtd/native/freebsd/shell_execute.hpp"
...
#  warning "The compiler is unknown, please check how to get command line arguments from the compiler."
  ...
  const distribution_dictionary& get_distribution_key_values() {
    ...
    auto distribution_string = freebsd::shell_execute::run("cat", "/etc/os-release");
    auto distribution_lines = xtd::native::freebsd::strings::split(distribution_string, {'\n'});
    for (auto distribution_line : distribution_lines) {
      auto key_value = xtd::native::freebsd::strings::split(distribution_line, {'='});
      if (key_value.size() != 2) continue;
      distribution_key_values.insert({key_value[0], xtd::native::freebsd::strings::replace(key_value[1], "\"", "")});
    }
...
string environment::get_desktop_environment() {
  ...
  for (auto environment_desktop : {"budgie", "cinnamon", "deepin", "Enlightenment", "étoilé", "gnome", "kde", "lxqt", "mate", "pantheon", "razor", "unity", "xfce"}) {
    if (freebsd::strings::contains(freebsd::strings::to_lower(current_desktop), environment_desktop)) return environment_desktop;
  }
  return "";
}
...
string environment::get_desktop_theme() {
  ...
  auto current_theme = freebsd::shell_execute::run("gsettings", "get org.gnome.desktop.interface gtk-theme");
  if (current_theme.size() >= 4)
    current_theme = freebsd::strings::substring(current_theme, 1, current_theme.size() - 3);
...
vector<string> environment::get_distribution_like_ids() {
  ...
  if (iterator == get_distribution_key_values().end()) return {};
  return freebsd::strings::split(iterator->second, {' '});
}
...
void environment::get_distribution_version(int32_t& major, int32_t& minor, int32_t& build, int32_t& revision) {
  ...
  if (name_it == get_distribution_key_values().end()) return;
  auto versions = xtd::native::freebsd::strings::split(name_it->second, {'.'});
  ...
  if (target == ENVIRONMENT_VARIABLE_TARGET_PROCESS) {
    ...
    if (envs.size() == 0) {
      for (auto index = size_t {0}; environ[index] != nullptr; index++) {
        vector<string> key_value = freebsd::strings::split(environ[index], {'='});
        if (key_value.size() == 2)
...
string environment::get_machine_name() {
  return freebsd::strings::replace(freebsd::shell_execute::run("uname", "-n"), "\n", "");
}

int32_t environment::get_os_platform_id() {
  if (freebsd::shell_execute::run("uname", "-a").find("FreeBSD") != string::npos) return PLATFORM_FREEBSD;
  return PLATFORM_UNIX;
}

void environment::get_os_version(int32_t& major, int32_t& minor, int32_t& build, int32_t& revision) {
  auto numbers = freebsd::strings::split(freebsd::shell_execute::run("uname", "-r"), {'.', '-', '\n'});
  if (numbers.size() < 1 || !freebsd::strings::try_parse(numbers[0], major)) major = 0;
  if (numbers.size() < 2 || !freebsd::strings::try_parse(numbers[1], minor)) minor = 0;
  if (numbers.size() < 3 || !freebsd::strings::try_parse(numbers[2], build)) build = 0;
  if (numbers.size() < 4 || !freebsd::strings::try_parse(numbers[3], revision)) revision = 0;
}
...
string environment::get_user_domain_name() {
  return freebsd::strings::trim_end(freebsd::shell_execute::run("uname", "-n"), {'\n'});
}
...
bool environment::has_shutdown_started() {
  // return always false on freebsd.
  return false;
}

bool environment::is_processor_arm() {
  auto uname_result = freebsd::shell_execute::run("uname", "-m");
  return freebsd::strings::contains(uname_result, "arm") || freebsd::strings::contains(uname_result, "aarch64");
}

bool environment::is_os_64_bit() {
  return freebsd::strings::contains(freebsd::shell_execute::run("uname", "-m"), "64");
}
