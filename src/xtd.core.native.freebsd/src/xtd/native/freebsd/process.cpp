...
intmax_t process::shell_execute(const std::string& verb, const string& file_name, const string& arguments, const string& working_directory, int32_t process_window_style) {
  pid_t process = fork();
  if (process == 0) {
    bool is_shell_execute = is_valid_shell_execute_process(&freebsd::strings::split, file_name, working_directory);
    if (is_shell_execute) {
      for (auto arg : split_arguments(arguments))
        if (!(is_shell_execute = is_valid_shell_execute_process(&freebsd::strings::split, arg, working_directory))) break;
    }
    ...
    if (is_shell_execute) {
      if (verb == "" || verb == "open" || verb == "explore" || verb == "edit" || verb == "runas" || verb == "runasuser" || verb == "print") {
        ...
        command_line_args.insert(command_line_args.begin(), get_full_file_name_with_extension(&freebsd::strings::split, file_name, working_directory));
      ...
      command_line_args.insert(command_line_args.begin(), get_full_file_name_with_extension(&freebsd::strings::split, file_name));
    }
...
process::started_process process::start(const string& file_name, const string& arguments, const string& working_directory, int32_t process_window_style, int32_t process_creation_flags, tuple<bool, bool, bool> redirect_standard_streams) {
  if (!is_valid_process(&freebsd::strings::split, file_name, working_directory)) return make_tuple(0, 0, std::unique_ptr<std::ostream> {}, std::unique_ptr<std::istream> {}, std::unique_ptr<std::istream> {});
