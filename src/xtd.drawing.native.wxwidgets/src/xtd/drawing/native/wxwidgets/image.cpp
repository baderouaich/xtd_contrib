...
void image::set_resolution(intptr image, int32 x_dpi, int32 y_dpi) {
  reinterpret_cast<wxImage*>(image)->SetOption(wxIMAGE_OPTION_RESOLUTIONUNIT, string::format("{}", wxIMAGE_RESOLUTION_INCHES).chars().c_str());
  reinterpret_cast<wxImage*>(image)->SetOption(wxIMAGE_OPTION_RESOLUTIONX, string::format("{}", x_dpi).chars().c_str());
  reinterpret_cast<wxImage*>(image)->SetOption(wxIMAGE_OPTION_RESOLUTIONY, string::format("{}", y_dpi).chars().c_str());
}
