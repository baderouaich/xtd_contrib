...
#include "../include/xtd/chrono.hpp"
...
time_zone_info::transition_time time_zone_info::transition_time::create_fixed_date_rule(date_time time_of_day, uint32 month, uint32 day) {
  if (time_of_day.year() != 1 || time_of_day.month() != 1 || time_of_day.day() != 1 || time_of_day.kind() != date_time_kind::unspecified) throw_helper::throws(exception_case::argument);
  if (month < 1 || month > 12 || day < 1 || day > 31) throw_helper::throws(exception_case::argument_out_of_range);
...
time_zone_info::transition_time time_zone_info::transition_time::create_floating_date_rule(date_time time_of_day, uint32 month, uint32 week, xtd::day_of_week day_of_week) {
  if (time_of_day.year() != 1 || time_of_day.month() != 1 || time_of_day.day() != 1 || time_of_day.kind() != date_time_kind::unspecified) throw_helper::throws(exception_case::argument);
  if (month < 1 || month > 12 || week < 1 || week > 5) throw_helper::throws(exception_case::argument_out_of_range);
  auto result = transition_time {};
...
const list<time_zone_info>& time_zone_info::get_system_time_zones() noexcept {
  ...
  std::for_each(stzs.begin(), stzs.end(), [&](const auto & item) {system_time_zones.add(time_zone_info(item.id, ticks(item.base_utc_offset), item.daylight_name, item.display_name, item.standard_name, item.supports_daylight_saving_time));});
