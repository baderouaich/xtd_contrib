...
namespace xtd {
  ...
  public:
    ...
    /// @return Pointer to the underlying element storage. For non-empty containers, the returned pointer compares equal to the address of the first element.
    /// @remarks The pointer is such that range [xtd::array::data(), xtd::array::data() + xtd::array::size()) is always a valid range, even if the container is empty (xtd::array::data() is not dereferenceable in that case).
    [[nodiscard]] virtual auto data() const noexcept -> const_pointer override {return (pointer)data_->items.data();}
...
for (const std::initializer_list<type_t>& il2 : il1)
  ...
  auto add(const type_t& item) -> void override {}
    ...
    [[nodiscard]] static auto compute_index(const xtd::basic_array<value_t>& items, const xtd::array<xtd::index>& indexes) -> xtd::usize;
